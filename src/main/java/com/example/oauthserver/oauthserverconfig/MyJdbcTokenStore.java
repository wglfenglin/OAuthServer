...
public class MyJdbcTokenStore implements TokenStore {
    ...
    private static final String DEFAULT_ACCESS_TOKEN_INSERT_STATEMENT = "insert into tbl_oauth_access_token (token_id, token, authentication_id, user_name, client_id, authentication, refresh_token) values (?, ?, ?, ?, ?, ?, ?)";

    private static final String DEFAULT_ACCESS_TOKEN_SELECT_STATEMENT = "select token_id, token from tbl_oauth_access_token where token_id = ?";

    private static final String DEFAULT_ACCESS_TOKEN_AUTHENTICATION_SELECT_STATEMENT = "select token_id, authentication from tbl_oauth_access_token where token_id = ?";

    private static final String DEFAULT_ACCESS_TOKEN_FROM_AUTHENTICATION_SELECT_STATEMENT = "select token_id, token from tbl_oauth_access_token where authentication_id = ?";

    private static final String DEFAULT_ACCESS_TOKENS_FROM_USERNAME_AND_CLIENT_SELECT_STATEMENT = "select token_id, token from tbl_oauth_access_token where user_name = ? and client_id = ?";

    private static final String DEFAULT_ACCESS_TOKENS_FROM_USERNAME_SELECT_STATEMENT = "select token_id, token from tbl_oauth_access_token where user_name = ?";

    private static final String DEFAULT_ACCESS_TOKENS_FROM_CLIENTID_SELECT_STATEMENT = "select token_id, token from tbl_oauth_access_token where client_id = ?";

    private static final String DEFAULT_ACCESS_TOKEN_DELETE_STATEMENT = "delete from tbl_oauth_access_token where token_id = ?";

    private static final String DEFAULT_ACCESS_TOKEN_DELETE_FROM_REFRESH_TOKEN_STATEMENT = "delete from tbl_oauth_access_token where refresh_token = ?";

    private static final String DEFAULT_REFRESH_TOKEN_INSERT_STATEMENT = "insert into tbl_oauth_refresh_token (token_id, token, authentication) values (?, ?, ?)";

    private static final String DEFAULT_REFRESH_TOKEN_SELECT_STATEMENT = "select token_id, token from tbl_oauth_refresh_token where token_id = ?";

    private static final String DEFAULT_REFRESH_TOKEN_AUTHENTICATION_SELECT_STATEMENT = "select token_id, authentication from tbl_oauth_refresh_token where token_id = ?";

    private static final String DEFAULT_REFRESH_TOKEN_DELETE_STATEMENT = "delete from tbl_oauth_refresh_token where token_id = ?";
    ...
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        ...
        // 放在 TokenEnhancer.enhance()里面时，clientId不为client不执行enhance()导致参数无法返回
        if (principal instanceof String) {
            mapRule(token, (String) principal);
        } else if(principal instanceof MyUser) {
            mapRule(token,  ((MyUser) principal).getUsername());
        } else{
            token.getAdditionalInformation().put("roles",StringUtils.EMPTY);
        }
//        if (principal instanceof MyUser) {
//            MyUser principal1 = (MyUser) principal;
//            token.getAdditionalInformation().put("roles",principal1.getSysAuthorities());
//        } else {
//            token.getAdditionalInformation().put("roles", StringUtils.EMPTY);
//        }
    ...
    public Collection<OAuth2AccessToken> findTokensByClientId(String clientId) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromClientIdSql, new MyJdbcTokenStore.SafeAccessTokenRowMapper(),
                    clientId);
        } catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled()) {
    ...
    public Collection<OAuth2AccessToken> findTokensByUserName(String userName) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromUserNameSql, new MyJdbcTokenStore.SafeAccessTokenRowMapper(),
                    userName);
        } catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled())
    ...
    public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
        ...
        try {
            accessTokens = jdbcTemplate.query(selectAccessTokensFromUserNameAndClientIdSql, new MyJdbcTokenStore.SafeAccessTokenRowMapper(),
                    userName, clientId);
        } catch (EmptyResultDataAccessException e) {
            if (LOG.isInfoEnabled()) {
    ...
    // mapping user rules from database
    private void mapRule(OAuth2AccessToken token,String principal){
        QueryWrapper<UserAuthority> userAuthorityQueryWrapper= new QueryWrapper<UserAuthority>()
                .eq("USER_CODE", principal);
        List<String> sysAuthorities = userAuthorityDao.selectList(userAuthorityQueryWrapper).stream().map(u->u.getAuthority()).collect(Collectors.toList());
        token.getAdditionalInformation().put("roles",sysAuthorities);
    }
}
