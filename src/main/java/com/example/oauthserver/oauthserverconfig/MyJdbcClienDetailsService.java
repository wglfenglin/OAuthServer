 ...
 * @date 2019/8/19 9:39
 */
public class MyJdbcClienDetailsService implements ClientDetailsService, ClientRegistrationService {
    private static final Log logger = LogFactory.getLog(MyJdbcClienDetailsService.class);

    private MyJdbcClienDetailsService.JsonMapper mapper = createJsonMapper();
    ...
    private static final String BASE_FIND_STATEMENT = "select client_id, " + CLIENT_FIELDS
            + " from tbl_oauth_client_details";

    private static final String DEFAULT_FIND_STATEMENT = BASE_FIND_STATEMENT + " order by client_id";

    private static final String DEFAULT_SELECT_STATEMENT = BASE_FIND_STATEMENT + " where client_id = ?";

    private static final String DEFAULT_INSERT_STATEMENT = "insert into tbl_oauth_client_details (" + CLIENT_FIELDS
            + ", client_id) values (?,?,?,?,?,?,?,?,?,?,?)";

    private static final String DEFAULT_UPDATE_STATEMENT = "update tbl_oauth_client_details " + "set "
            + CLIENT_FIELDS_FOR_UPDATE.replaceAll(", ", "=?, ") + "=? where client_id = ?";

    private static final String DEFAULT_UPDATE_SECRET_STATEMENT = "update tbl_oauth_client_details "
            + "set client_secret = ? where client_id = ?";

    private static final String DEFAULT_DELETE_STATEMENT = "delete from tbl_oauth_client_details where client_id = ?";

    private RowMapper<ClientDetails> rowMapper = new MyJdbcClienDetailsService.ClientDetailsRowMapper();
    ...
    public MyJdbcClienDetailsService(DataSource dataSource) {
    ...
    public ClientDetails loadClientByClientId(String clientId) throws InvalidClientException {
        ...
        try {
            details = jdbcTemplate.queryForObject(selectClientDetailsSql, new MyJdbcClienDetailsService.ClientDetailsRowMapper(), clientId);
        }
    ...
    private static MyJdbcClienDetailsService.JsonMapper createJsonMapper() {
        if (ClassUtils.isPresent("org.codehaus.jackson.map.ObjectMapper", null)) {
            return new MyJdbcClienDetailsService.JacksonMapper();
        }
        else if (ClassUtils.isPresent("com.fasterxml.jackson.databind.ObjectMapper", null)) {
            return new MyJdbcClienDetailsService.Jackson2Mapper();
        }
        return new MyJdbcClienDetailsService.NotSupportedJsonMapper();
    }

    private static class JacksonMapper implements MyJdbcClienDetailsService.JsonMapper {
    ...
    private static class Jackson2Mapper implements MyJdbcClienDetailsService.JsonMapper {
    ...
    private static class NotSupportedJsonMapper implements MyJdbcClienDetailsService.JsonMapper {
        @Override
        public String write(Object input) throws Exception {
